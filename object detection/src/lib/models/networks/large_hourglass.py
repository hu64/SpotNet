...
def make_saliency_layer(curr_dim):  # hughes
    return nn.Sequential(
        # SqEx(curr_dim),
...
class exkp(nn.Module):
    def __init__(
        ...
        for head in heads.keys():
            ...
            elif 'seg' in head and (not STD_CENTERNET): # hughes
                module = nn.ModuleList([
                    make_saliency_layer(curr_dim) for _ in range(nstack)
                    # segmentation_module(curr_dim) for _ in range(nstack)
                ])
                self.__setattr__(head, module)
                # for heat in self.__getattr__(head):
                #     heat[-1].bias.data.fill_(-2.19)
        ...
        for ind in range(self.nstack):
            ...
            for head in self.heads:
                if not head == seg_name:
                    ...
                    if head == 'hm':
                        if STD_CENTERNET:
                            y = cnv  # * nn.MaxPool2d(4)(out[seg_name])
                        else:
                            attention = nn.MaxPool2d(4)(out[seg_name])
                            y = cnv * attention
                        # print(self.__getattr__('SqEx'))
                        # y = self.__getattr__('SqEx')(y)
                        y = layer(y)
                        out[head] = y
                    else:
                        y = layer(cnv)
